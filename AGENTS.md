...
- **Audio mudo**: el `<audio>` element (video muteado por diseño) depende de `audioSrc` del track extraído. El sync de `canplay` debe ser PERMANENTE (no `{ once: true }`): con once, si el `audio.play()` se intenta antes de que el elemento esté listo (NotSupportedError silenciado por el catch) el audio queda mudo para siempre; el canplay permanente reintenta. Fallback en `onAudioError`: `video.muted = false` (audio nativo del video).
